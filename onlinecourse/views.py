...
def show_exam_result(request, course_id, submission_id):
    ...
    # For each question in the course, check if the selected choices are correct
    results = []
    total_score = 0
    for question in course.question_set.all():
        ...
        question_selected_ids = [cid for cid in selected_choice_ids if Choice.objects.get(id=cid).question == question]
        question_results['selected_choices'] = Choice.objects.filter(id__in=question_selected_ids)
        
        # Check if all correct choices are selected and no incorrect
        correct_choices = question.choice_set.filter(is_correct=True)
        correct_ids = set(correct_choices.values_list('id', flat=True))
        selected_set = set(question_selected_ids)
